        ...
        secao.content.forEach(item => {
            ...
            if (item.type === "accordion") {
                item.items.forEach(acc => {
                    ...
                    // CORREÇÃO UX: Adicionado "white-space: pre-wrap; word-break: break-word;" exclusivamente no <pre> do Markdown
                    if (acc.markdown && acc.python) {
                        body.innerHTML = `
                            <div style="margin-bottom: 1.5rem;">
                                <div style="font-size: 0.85rem; font-weight: 700; color: var(--secondary); margin-bottom: 0.5rem;"><i class="fa-solid fa-align-left"></i> CÉLULA TEXTUAL (MARKDOWN)</div>
                                <pre style="white-space: pre-wrap; word-break: break-word;"><code>${acc.markdown}</code></pre>
                            </div>
                            <div>
                                <div style="font-size: 0.85rem; font-weight: 700; color: var(--primary); margin-bottom: 0.5rem;"><i class="fa-brands fa-python"></i> CÉLULA DE CÓDIGO (PYTHON)</div>
                                <pre><code>${acc.python}</code></pre>
                            </div>
                        `;
                    } else {
...
function renderizarAbaRevisao(secao, container) {
    ...
    cardNorteadores.innerHTML = `<h2><i class="fa-solid fa-compass" style="color:var(--primary)"></i> Síntese Teórica (Perguntas Norteadoras)</h2>
                                 <p>Explore os 5 princípios fundamentais consolidados nesta aula:</p>`;
    ...
    topoQuiz.innerHTML = `<h2><i class="fa-solid fa-brain"></i> Auditoria de Conhecimento (Quiz)</h2>
                          <p>Teste sua retenção mecânica respondendo às 10 avaliações abaixo.</p>`;
